...
public class SurefireUtilsTest {
  ...
  public void should_get_reports_from_property() {
    ...
    when(configuration.get("sonar.junit.reportsPath")).thenReturn(Optional.of("target/surefire"));
    assertThat(SurefireUtils.getReportsDirectory(configuration, fs, pathResolver).exists()).isTrue();
    assertThat(SurefireUtils.getReportsDirectory(configuration, fs, pathResolver).isDirectory()).isTrue();
  }

  @Test
  public void return_default_value_if_property_unset() throws Exception {
    File directory = SurefireUtils.getReportsDirectory(mock(Configuration.class), fs, pathResolver);
  ...
  public void return_default_value_if_can_not_read_file() throws Exception {
    ...
    File directory = SurefireUtils.getReportsDirectory(configuration, fs, pathResolver);
    assertThat(directory.getCanonicalPath()).endsWith("target" + File.separator + "surefire-reports");
    assertThat(directory.exists()).isFalse();
    assertThat(directory.isDirectory()).isFalse();
  }
